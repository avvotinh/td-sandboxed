...
- `services/trading-engine/src/rules/violation_db_writer.py` (NEW) - RuleViolationModel ORM + ViolationDBWriter batch writer with max_buffer_size overflow protection
- `services/trading-engine/src/rules/violation_service.py` (NEW) - ViolationService facade with record_block(), record_warning()
- `services/trading-engine/src/rules/__init__.py` (MODIFIED) - Added violation tracking exports (RuleViolation, RuleViolationModel, ViolationDBWriter, ViolationService)
- `services/trading-engine/src/execution/order_validator.py` (MODIFIED) - Added violation_service injection, BLOCK/WARN recording in _log_rule_results_to_audit(), removed dead _audit_task_done method
- `services/trading-engine/src/engine.py` (MODIFIED) - Added _initialize_violation_tracking() for ViolationDBWriter lifecycle (start/stop)
...
- `services/trading-engine/tests/conftest.py` (MODIFIED) - Added shared FakeRule class and fake_rule fixture
...
| 2026-02-26 | Second code review fixes: (H1) Added violation exports to rules/__init__.py — RuleViolation, RuleViolationModel, ViolationDBWriter, ViolationService now follow Story 4.8 export pattern. (H2) Wired ViolationDBWriter lifecycle into engine.py — added _initialize_violation_tracking() with start/stop, matching DailySnapshotWriter pattern. (M1) Added max_buffer_size (default 10,000) to ViolationDBWriter with overflow protection on flush failure, preventing OOM under sustained DB outages. (M3) Removed dead _audit_task_done method from OrderValidator. (L1) Extracted FakeRule to shared tests/conftest.py. 42 tests, 0 regressions. | Claude Opus 4.6 |
