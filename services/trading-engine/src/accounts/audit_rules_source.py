"""Audit accounts' rule-source assignment.

Story 10.11 introduced this CLI as the gate ops signed off on before
Phase 5. Story 10.12 removed the legacy ``prop_firm`` preset source
entirely — the audit now classifies every account into one of three
buckets and the strict-check is effectively a structural sanity check.
...
class RulesSourceAuditReport:
    ...
    Attributes:
        ...
        prop_firm_account_total: Number of ``AccountType.PROP_FIRM`` rows
            in the report.
        prop_firm_legacy_count: Always 0 since 10.12 dropped the legacy
            preset path. Retained for backwards-compat with the 10.11
            CLI ``--strict`` exit-code contract — strict can no longer
            fail on this value, but external consumers parsing the
            JSON output don't break.
    """

    rows: tuple[AccountAuditRow, ...]
    counts: dict[RulesSource, int]
    prop_firm_account_total: int
    prop_firm_legacy_count: int = 0
