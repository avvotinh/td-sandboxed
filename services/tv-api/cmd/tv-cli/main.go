...
var (
	...
	bfResponseTimeoutMs = flag.Int("response-timeout-ms", 2000, "[backtest-fetch] timeout (ms) waiting for each request_more_data response. Raise to ≥5000 for M1 (initial replay batch is slow); 2000 suffices for M5+.")
)
...
func showHelp() {
	...
	fmt.Println("  # Server cap ≈ 5500 bars per -to anchor regardless of subscription:")
	fmt.Println("  #   M5  → ~25 calendar days back (premium ReplayMode)")
	fmt.Println("  #   M1  → ~4 trading days back (premium ReplayMode, needs -response-timeout-ms ≥ 5000)")
	fmt.Println("  # For multi-month windows, run multiple invocations with stepped")
	fmt.Println("  # -to anchors and merge via go_manifest_loader.")
	...
	fmt.Println()
	fmt.Println("  # Same shape for M1 — bump response timeout so the initial replay batch lands")
	fmt.Println("  tv-cli -command backtest-fetch -replay-mode -response-timeout-ms 10000 \\")
	fmt.Println("    -symbol OANDA:XAUUSD -timeframe 1 \\")
	fmt.Println("    -from 2026-04-25T00:00:00Z -to 2026-05-01T00:00:00Z \\")
	fmt.Println("    -window-name in_sample -window-kind in_sample \\")
	fmt.Println("    -out data/historical/XAUUSD/M1/chunk.parquet")
	fmt.Println()
